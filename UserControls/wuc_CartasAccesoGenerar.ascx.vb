...
Partial Public Class wuc_CartasAccesoGenerar
    Inherits System.Web.UI.UserControl

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If IsPostBack Then Exit Sub

        Page.Title = "GENERAR MEMO"
        ...
        For x As Integer = 0 To arr.Length - 1
            If arr(x) = "GENERAR CARTA DE ACCESO" Then
                Exit For
    ...
    Protected Sub rdb_Empleado_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles rdb_Empleado.CheckedChanged
        ...
        tbx_Empresa.Text = "SERVICIO INTEGRAL DE SEGURIDAD, S.A. DE C.V."
    ...
    Protected Sub btn_Agregar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_Agregar.Click
        ...
        If rdb_Empleado.Checked Then
            If ddl_Empleado.SelectedValue = "0" Then
                MostrarAlertAjax("Seleccione un Usuario para agregar a la lista.", ddl_Empleado, Page)
                ddl_Empleado.Focus()
                Exit Sub
            End If
            ...
            For Each fila As GridViewRow In gv_Empleados.Rows
                If gv_Empleados.DataKeys(fila.RowIndex).Values("Id_Empleado") = ddl_Empleado.SelectedValue Then
                    MostrarAlertAjax("Elemento seleccionado ya existe en la lista.", ddl_Empleado, Page)
                    Exit Sub
                End If
            Next
        Else
            If tbx_Nombre.Text = "" Then
                MostrarAlertAjax("Capture el Nombre.", ddl_Empleado, Page)
                tbx_Nombre.Focus()
                Exit Sub
            End If
            If tbx_Empresa.Text = "" Then
                MostrarAlertAjax("Capture el nombre de la Empresa.", ddl_Empleado, Page)
                tbx_Empresa.Focus()
                Exit Sub
            End If
        ...
        If Session("UsuariosAgregadosCarta") = "" Then
            'Session("UsuariosAgregadosCarta") = ddl_Empleado.SelectedValue & "," & ddl_Empleado.SelectedItem.Text
            Session("UsuariosAgregadosCarta") = PersonaAcreditada
        Else
            'Session("UsuariosAgregadosCarta") = Session("UsuariosAgregadosCarta") & ";" & ddl_Empleado.SelectedValue & "," & ddl_Empleado.SelectedItem.Text
    ...
    Sub ActualizarVisitantes(ByVal VEliminar As Integer)
        ...
        For x As Integer = 0 To Visitantes.Length - 1
            'Dim div() As String = Split(Visitantes(x), ",")
            If x <> VEliminar Then
    ...
    Protected Sub btn_Guardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_Guardar.Click
        ...
        If Session("UsuariosAgregadosCarta") = "" Then
            MostrarAlertAjax("Debe agregar al menos un Empleado/Visitante.", btn_Guardar, Page)
            Exit Sub
        End If
        If tbx_FechaInicio.Text = "" Then
            MostrarAlertAjax("Seleccione la Fecha de Inicio.", btn_Guardar, Page)
            tbx_FechaInicio.Focus()
            Exit Sub
        ElseIf tbx_FechaInicio.Text < DateTime.Now.Date Then
            MostrarAlertAjax("La Fecha de Inicio debe ser mayor o igual al día de hoy.", btn_Guardar, Page)
            tbx_FechaInicio.Focus()
            Exit Sub
        End If
        If Not rdb_NuevoIngreso.Checked And Not rdb_FaltaRetardo.Checked And Not rdb_Externo.Checked And Not rdb_Otro.Checked Then
            MostrarAlertAjax("Seleccione el Tipo de Carta.", btn_Guardar, Page)
            Exit Sub
        End If
        If tbx_Asunto.Text = "" Then
            MostrarAlertAjax("Capture el Asunto.", btn_Guardar, Page)
            tbx_Asunto.Focus()
            Exit Sub
        End If
        If ddl_Autoriza.SelectedIndex = 0 Then
            MostrarAlertAjax("Seleccione quién Autoriza la Carta.", btn_Guardar, Page)
            ddl_Autoriza.Focus()
            Exit Sub
        End If
        If ddl_Dirigida.SelectedIndex = 0 Then
            MostrarAlertAjax("Seleccione a quién va Dirigida la Carta.", btn_Guardar, Page)
            ddl_Dirigida.Focus()
            Exit Sub
        End If
        ...
        If Not fn_GenerarCartasAcceso_Nuevo(Session("SucursalID"), Session("UsuarioID"), Session("EstacioN"), Session("UsuariosAgregadosCarta"), tbx_Asunto.Text.ToUpper, ddl_Autoriza.SelectedValue, tbx_FechaInicio.Text, tbx_FechaInicio.Text, Tipo, ddl_Dirigida.SelectedValue, tbx_Empresa.Text) Then
            MostrarAlertAjax("Ha ocurrido un error al intentar Generar la Carta de Acceso.", btn_Guardar, Page)
        Else
            LimpiarPantalla()
            gv_Empleados.DataSource = fn_CreaGridVacio("Id_Empleado,Nombre")
            gv_Empleados.DataBind()
